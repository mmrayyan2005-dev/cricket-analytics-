...
def _get_remote_last_updated():
    """Fetch last_updated.txt from GitHub (cached for 1 hour)."""
    try:
...
def get_last_updated():
    return _get_remote_last_updated()

@st.cache_data(ttl=0, show_spinner=False)
def _get_data_version():
    """Returns the last_updated string to use as cache key — fetched fresh each time."""
    try:
        r = requests.get(f"{RAW_BASE}/last_updated.txt", timeout=5,
                         headers={"Cache-Control": "no-cache", "Pragma": "no-cache"})
        if r.status_code == 200:
...
@st.cache_data(ttl=3600, show_spinner=False)
def load(_version="static"):
    """Load all CSVs. Re-fetches when _version (last_updated) changes."""
    def read(name):
        try:
            r = requests.get(f"{RAW_BASE}/{name}",
                             headers={"Cache-Control": "no-cache", "Pragma": "no-cache"},
                             timeout=30)
            r.raise_for_status()
...
with st.spinner("Loading cricket data..."):
    _version = _get_data_version()
    (batting,bowling,bat_fmt,bowl_fmt,bat_yr,bowl_yr,bat_ven,bat_opp,
     bowl_ven,bowl_opp,bvb,wvb,bat_form,bowl_form,bat_sim,bowl_sim,bat_inn,bowl_inn)=load(_version)
