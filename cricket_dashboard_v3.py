...
CFG = dict(config={"displayModeBar":False,"scrollZoom":False,"responsive":True},
...
def bar_h(df,x,y,col,scale,title):
    ...
    fig.update_layout(**BASE,height=h,
        yaxis=dict(categoryorder="total ascending",color=TEXT,showgrid=False),
...
def bar_v(df,x,y,title,color,h=320):
    ...
    fig.update_layout(**BASE,height=h,showlegend=False,
        xaxis=dict(tickmode="linear",tickangle=-45,color=TEXT,showgrid=False))
    return fig
...
if section=="🔍 Player Search":
    ...
    if name:
        ab=bat_fmt[bat_fmt["striker"].str.contains(name,case=False,na=False)]["format"].unique().tolist()
        aw=bowl_fmt[bowl_fmt["bowler"].str.contains(name,case=False,na=False)]["format"].unique().tolist()
        ...
        if len(bat)>0:
            ...
            by=bat_yr[(bat_yr["striker"].str.contains(name,case=False,na=False))&(bat_yr["format"]==fmt)].sort_values("year")
            if len(by)>1:
        ...
        if len(bowl)>0:
            ...
            by2=bowl_yr[(bowl_yr["bowler"].str.contains(name,case=False,na=False))&(bowl_yr["format"]==fmt)].sort_values("year")
            if len(by2)>1:
